 ...
 *  Interpreter : ledit ocaml Lambda_Calcul.cmo
 *
 * DEMO at end of the this file
 *)
...
module Lambda_Term =
  struct
   ...
   (* REDUCTION OF LAMBDA TERM BY BETA REDUCTION *)
    ...
    (* Beta reduction = substitution that respects the scope of variables *)
    ...
    let i_reduce (*USER*) : t -> t = fun t ->
	  Trace.trace (fun _ -> reduce t) 
	    
    let demo (*USER*) : unit -> lambda_term list = fun () ->
...
(* DEMO 
   ...
   let _ = Lambda_Term.demo()
    
 * /!\ NON TERMINATING 
         
    let _ = Lambda_Term.i_reduce Lambda_Term.diverge ;; 
 *)
