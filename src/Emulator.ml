...
!{ This is the .mic file that must be used for any change. Don't mofidy the .ml file which is automatically generated from .mic.ml. Your modifications would be lost. }!
    ...
    let rec (execute_action_using: simulators * loggers -> (State.t * Action.t * State.t) -> Configuration.t -> Configuration.t) = fun (simulators,loggers) (src,action,tgt) cfg ->
      ...
      let next_bands =
        ...
        | simulator :: other_simulators
          ...
          and emulation_bands = (simulator.encoder cfg.bands) >> (show_bands_using loggers (String.concat " to " [ "encoding" ; simulator.name ]))
          in let emulation_cfg = Configuration.make emulation_tm emulation_bands
          in
          let e_next_cfg = log_run_using (other_simulators,loggers) emulation_cfg
          in
          ...
          in
          if (* FIXME: Band.are_equivalents (regardless of starting or ending B) instead of = *) bands_updated_by_execution = bands_updated_by_emulation
...
struct
  ...
  let decode_with : encoding -> Band.t list -> Band.t list = fun encoding bands ->
    List.map (Band.map (function S -> U | D -> Z)) bands


  (* EMULATION OF TRANSITIONS *)

  let (emulate_action: State.t * Action.t * State.t -> Turing_Machine.t) = fun (source,action,target) ->
      let action = Action.map (function U -> S | Z -> D) action in
      let (source,target) =
      if source <> target   (* /!\ loop in the emulated TM if source=target *)
      then (source,target)
