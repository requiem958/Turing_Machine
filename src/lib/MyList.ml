 ...
 * CONTENT 
 *
 *   Extension of the List module with additional functions
 *
 * USAGE
 *
 *   Require    :  
 *   Compilation:  ocamlc MyList.ml 
 *   Interpreter:  ledit ocaml MyList.cmo 
 *)
...
let make : int -> 'a -> 'a list
  = fun n elt ->
    iterate n (fun acc -> elt::acc) []

let integers_from_to : int -> int -> int list
  = fun first last ->
    let rec trec_integers_from_to : int list -> int -> int list
      = fun acc i ->
        if i<first then acc
        else trec_integers_from_to (i::acc) (i-1)
    in trec_integers_from_to [] last
...
let rec but_last : 'a list -> 'a list = fun l ->
  match l with
  | [] -> failwith "MyList.but_last: empty list"
  | [x] -> []
  | x::xs -> x :: (but_last xs)
...
let number_from : int -> 'a list -> (int  * 'a) list
  = fun n l ->
    number_with (fun n x -> (n,x)) (fun n x -> (n+1)) n l
