 ...
 *
 *   Requirement
 *    - Module  :  MyList.cmo MyString.cmo Pretty.cmo Color.cmo 
 *    - Library :
 *   Compilation:  ocamlc      MyList.cmo MyString.cmo Pretty.cmo Color.cmo Html.ml 
 *   Interpreter:  ledit ocaml MyList.cmo MyString.cmo Pretty.cmo Color.cmo Html.cmo
 *)
...
let old_wide_cell: int -> int -> cell = fun width n ->
      if n>0
      then (cell [("width", Int width) ; ("bgcolor", Option "orange") ] "")
           |> (MyList.make n)
           |> (String.concat "")
      else ""
