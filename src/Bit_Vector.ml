 ...
 *    - Library :
 *   Compilation:  ocamlc      MyList.cmo MyString.cmo Tricks.cmo Pretty.cmo Bit_Vector.ml
 *   Interpreter:  ledit ocaml MyList.cmo MyString.cmo Tricks.cmo Pretty.cmo Bit_Vector.cmo
 *
 * DEMO 
 *
...
module Made_Of = functor (Bit : Two_Values_Sig) ->
  (struct
    ...
    let bits_to_int : t -> int option
      = fun bits ->
        let rec horner : int -> t -> int
          = fun int ->
            function
            | [] -> int
            | b::bits ->
    ...
    let rec enumerate_from_to : int -> int -> bit vectors
      = fun init last ->
        if init>last
        then []
    ...
    let rec enumerate_x_from : int -> bit vector -> bit vectors
      = fun counter initial_bit_vector ->
        if counter<=0 then []
        else initial_bit_vector :: (enumerate_x_from (counter-1) (inc initial_bit_vector))

    let enumerate : int -> bit vectors
      = fun n ->
...
module Demo =
  (struct
    ...
    module Bit = Bit_as_Boolean
    module BV = Made_Of(Bit)
    open BV

    (* TEST *)

    let check_operations_on : int -> unit
      = fun i ->
      begin
