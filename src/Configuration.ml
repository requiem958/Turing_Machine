 ...
 * USAGE
 *
 *   Require    :  Tricks.ml Band.ml State.ml Action.ml Transition.ml Turing_Machine.ml 
...
module Configuration =
  (struct
    ...
    let make_filename: Turing_Machine.t -> Band.t list -> string = fun tm bands ->
      String.concat "" [ "TM_" ; tm.name ; "_" ; string_of_int tm.nb_bands ; "x" ;  if tm.nb_bands>1 then "Bands" else "Band" ; Pretty.get_extension() ]

      
    let make ?time:(time=false) : (Turing_Machine.t -> Band.t list -> configuration) = fun tm bands ->
      let
	filename = (* if time then begin Date.sleep_for 0.1 ; String.concat "_" [ Date.pretty_time() ; name tm bands ] end else *) make_filename tm bands 
      in
      { tm     = tm ;
	bands  = bands ;
	state  = tm.initial ;
 ...
 *)
    ...
    let do_n_times: int -> ('t -> unit) -> 't list -> unit = fun n f ts ->
    ...
    let print_using ?n_first:(n_first=(-1)) : (Logger.t list -> t -> unit) = fun loggers cfg ->
      ...
      if n_first<0
      then List.iter log_cfg loggers (*forall*)
      else do_n_times n_first log_cfg loggers 
...
let demo: unit -> unit = fun () ->
  ...
  let band1 = Band.make "B1" alphabet [B;D;Z;U;U;Z;B]
  and band2 = Band.make "B2" alphabet [B;B;B;L;B;B;B] in
